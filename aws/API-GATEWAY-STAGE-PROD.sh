...
echo "Recurso /clients created: $CLIENTS_RES_ID"
...
aws --endpoint-url=$LS_URL apigateway put-method \
  --rest-api-id $API_ID \
  --region $REGION \
  --resource-id $ACCOUNTS_ACCOUNTS_RES_ID \
  --http-method POST \
  --authorization-type "NONE"
...
aws --endpoint-url=$LS_URL apigateway create-deployment \
  --rest-api-id $API_ID --region $REGION --stage-name prod

echo "API Gateway configured successfully!"
echo "Base URL: $LS_URL/restapis/$API_ID/prod/_user_request_/clients"
